...
fn show_load_with_env_overrides() {
	...
	match InklogConfig::load_with_env_overrides() {
		Ok(config) => {
			...
			println!("  performance.channel_capacity  = {}", config.performance.channel_capacity);
			println!("  performance.worker_threads    = {}", config.performance.worker_threads);
			println!("  performance.channel_strategy  = {:?}", config.performance.channel_strategy);
			println!(
				"\n  file_sink.enabled     = {}",
...
fn show_env_override_demo() {
	...
	println!("  performance.channel_capacity = {}", baseline.performance.channel_capacity);
	println!("  performance.worker_threads = {}", baseline.performance.worker_threads);
	...
	println!("\n覆盖后配置：");
	println!("  global.level = \"{}\" (期望: \"debug\")", overridden.global.level);
	println!("  performance.channel_capacity = {} (期望: 20000)", overridden.performance.channel_capacity);
	println!("  performance.worker_threads = {} (期望: 4)", overridden.performance.worker_threads);
	...
	println!("\n验证结果：");
	println!("  global.level 覆盖: {} → {}", baseline.global.level != overridden.global.level, if level_ok { "✓" } else { "✗" });
	println!("  channel_capacity 覆盖: {} → {}", baseline.performance.channel_capacity != overridden.performance.channel_capacity, if capacity_ok { "✓" } else { "✗" });
	println!("  worker_threads 覆盖: {} → {}", baseline.performance.worker_threads != overridden.performance.worker_threads, if workers_ok { "✓" } else { "✗" });
