...
fn show_strategy_variants() {
	...
	println!("\n默认策略：");
	println!("  ChannelStrategy::default() = {:?}", ChannelStrategy::default());

	println!("\n行为对比：");
	println!("{:<15} {:<25} {:<25}", "场景", "Fixed", "Adaptive");
	println!("{}", "-".repeat(65));
	println!("{:<15} {:<25} {:<25}", "正常负载", "稳定", "稳定（动态变化）");
	println!("{:<15} {:<25} {:<25}", "流量突增", "可能丢弃日志", "扩容以处理");
	println!("{:<15} {:<25} {:<25}", "低流量", "容量恒定", "缩容以节省内存");
	println!("{:<15} {:<25} {:<25}", "内存使用", "可预测", "可变（通常更低）");
}
...
fn show_adaptive_thresholds() {
	...
	println!("  expand_threshold_percent= {}", default.expand_threshold_percent);
	println!("  shrink_threshold_percent= {}", default.shrink_threshold_percent);
	println!("  shrink_wait_seconds     = {}", default.shrink_wait_seconds);
	...
	println!("  channel_capacity        = {}", aggressive.channel_capacity);
	println!("  channel_strategy        = {:?}", aggressive.channel_strategy);
	println!("  expand_threshold_percent= {} (更早扩容)", aggressive.expand_threshold_percent);
	println!("  shrink_threshold_percent= {} (更晚缩容)", aggressive.shrink_threshold_percent);
	println!("  shrink_wait_seconds     = {} (更长防抖)", aggressive.shrink_wait_seconds);
...
fn show_performance_profiles() {
	...
	let profiles = [
		("高吞吐量 (>10k logs/s)", PerformanceConfig {
		...
		("资源受限 (有限 RAM/CPU)", PerformanceConfig {
		...
		("突发流量 (Adaptive 推荐)", PerformanceConfig {
