...
fn show_size_based_rotation() {
	...
	println!("max_size = {} 字节 ({} MB)", strategy.max_size(), strategy.max_size() / 1024 / 1024);
	...
	println!("current_size = {} MB, 限制 = 100 MB", ctx.current_size / 1024 / 1024);
...
fn show_time_based_rotation() {
	...
	assert_eq!(
		TimeBasedRotation::from_interval_string("hourly").unwrap().interval_secs(),
		3600
	);
	assert_eq!(
		TimeBasedRotation::from_interval_string("daily").unwrap().interval_secs(),
		86400
	);
...
fn show_composite_rotation() {
	...
	println!("size=100B(<1KB), elapsed=1h(<24h) → should_rotate={}", result.should_rotate);
	assert!(!result.should_rotate);

	print_section("4.2 size 条件满足 → 触发轮转");
	let ctx = make_context(2048, 3600, 0); // 2048 字节 > 1024
	let result = composite.should_rotate(&ctx);
	println!("size=2048B(>1KB), elapsed=1h(<24h) → should_rotate={}", result.should_rotate);
	assert!(result.should_rotate);

	print_section("4.3 time 条件满足 → 触发轮转");
	let ctx = make_context(100, 86400, 0); // 100 字节，24 小时
	let result = composite.should_rotate(&ctx);
	println!("size=100B(<1KB), elapsed=24h(>=24h) → should_rotate={}", result.should_rotate);
	assert!(result.should_rotate);
}
...
fn show_file_sink_config_rotation() {
	...
	println!("      任一条件满足即触发轮转，轮转后文件按 {{stem}}_{{timestamp}}_{{seq}}.{{ext}} 命名。");
}
