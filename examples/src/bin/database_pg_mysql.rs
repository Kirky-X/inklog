...
fn show_driver_variants() {
	...
	println!("\n默认驱动：");
	println!("  DatabaseDriver::default() = {:?}", DatabaseDriver::default());
}
...
fn show_from_str_display() {
	...
	let inputs = ["postgres", "postgresql", "PostgreSQL", "mysql", "MySQL", "sqlite", "sqlite3", "SQLite"];
	for input in inputs {
	...
	println!("\nDisplay 输出：");
	for driver in [DatabaseDriver::PostgreSQL, DatabaseDriver::MySQL, DatabaseDriver::SQLite] {
...
fn show_driver_comparison() {
	...
	println!("{:<12} {:<15} {:<20} {:<15}", "驱动", "默认端口", "适用场景", "推荐度");
	println!("{}", "-".repeat(65));
	println!("{:<12} {:<15} {:<20} {:<15}", "PostgreSQL", "5432", "企业级生产", "★★★★★");
	println!("{:<12} {:<15} {:<20} {:<15}", "MySQL", "3306", "Web 应用", "★★★★");
	println!("{:<12} {:<15} {:<20} {:<15}", "SQLite", "无", "开发/测试/嵌入式", "★★★");

	println!("\n功能对比：");
	println!("{:<20} {:<12} {:<12} {:<12}", "特性", "PostgreSQL", "MySQL", "SQLite");
	println!("{}", "-".repeat(56));
	println!("{:<20} {:<12} {:<12} {:<12}", "分区表", "✓", "✓", "✗");
	println!("{:<20} {:<12} {:<12} {:<12}", "JSONB", "✓", "✗ (JSON)", "✗");
	println!("{:<20} {:<12} {:<12} {:<12}", "并发写入", "高 (MVCC)", "中", "低 (锁)");
	println!("{:<20} {:<12} {:<12} {:<12}", "连接池", "必需", "必需", "N/A");
