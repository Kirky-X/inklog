...
fn show_auth_config() {
	...
	println!("  2. 服务端从环境变量 {} 读取期望的 token", enabled_auth.token_env);
...
fn show_error_modes() {
	...
	println!("{:<15} {:<30} {:<30}", "缺失认证", "返回数据 + 警告 header", "返回 401 Unauthorized");
	println!("{:<15} {:<30} {:<30}", "无效认证", "返回数据 + 警告 header", "返回 401 Unauthorized");
	println!("{:<15} {:<30} {:<30}", "IP 不在白名单", "返回数据 + 警告 header", "返回 403 Forbidden");

	println!("\n默认模式：");
	println!("  HttpErrorMode::default() = {:?}", HttpErrorMode::default());
...
fn show_full_secure_config() {
	...
	println!("  host:port    = {}:{}", secure_config.host, secure_config.port);
	...
	println!("  auth.enabled = {}", secure_config.auth.as_ref().map(|a| a.enabled).unwrap_or(false));
	println!("  auth.token_env = \"{}\"",
		secure_config.auth.as_ref().map(|a| a.token_env.as_str()).unwrap_or(""));
	println!("  ip_whitelist = {:?}", secure_config.ip_whitelist);

	println!("\n对应的 TOML 配置：");
	println!(r#"[http_server]
enabled = true
host = "0.0.0.0"
port = 9090
