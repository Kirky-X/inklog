...
fn show_parquet_config() {
	...
	println!("  compression_level  = {} (Zstandard 0-10)", default.compression_level);
	println!("  encoding           = \"{}\"", default.encoding);
	println!("  max_row_group_size = {} (行/行组)", default.max_row_group_size);
	...
	println!("  compression_level  = {} (最大压缩)", high_compression.compression_level);
	println!("  encoding           = \"{}\"", high_compression.encoding);
	println!("  max_row_group_size = {} (更大行组)", high_compression.max_row_group_size);
	println!("  max_page_size      = {} (2MB)", high_compression.max_page_size);
	...
	println!("  compression_level  = {} (快速)", low_latency.compression_level);
	println!("  encoding           = \"{}\"", low_latency.encoding);
	println!("  max_row_group_size = {} (更小行组)", low_latency.max_row_group_size);
	println!("  max_page_size      = {} (512KB)", low_latency.max_page_size);
}
...
fn show_convert_logs_to_parquet() {
	...
	let records = vec![
		...
		LogRecord::new(Level::WARN, "module_a".to_string(), "配置项缺失，使用默认值".to_string()),
		LogRecord::new(Level::ERROR, "module_b".to_string(), "数据库连接失败".to_string()),
		LogRecord::new(Level::INFO, "module_c".to_string(), "请求处理完成".to_string()),
	];
	...
	match convert_logs_to_parquet(&records, &config) {
		Ok(parquet_bytes) => {
			...
			println!("  压缩比: {:.2} bytes/record", parquet_bytes.len() as f64 / records.len() as f64);

			// 验证 Parquet magic number (PAR1)
			if parquet_bytes.len() >= 4 {
				let magic = &parquet_bytes[..4];
				println!("  Parquet magic: {:?} (期望: [80, 65, 82, 49] = \"PAR1\")", magic);
			}
		}
		Err(e) => {
			println!("  ✗ 转换失败: {}", e);
...
fn show_database_sink_with_parquet() {
	...
	println!("  archive_format   = \"{}\" ← 启用 Parquet 归档", config.archive_format);
	println!("  parquet_config.compression_level  = {}", config.parquet_config.compression_level);
	println!("  parquet_config.max_row_group_size = {}", config.parquet_config.max_row_group_size);

	println!("\n对应 TOML 配置：");
	println!(r#"[database_sink]
enabled = true
driver = "sqlite"
