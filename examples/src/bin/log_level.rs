...
fn show_from_str_parsing() {
	...
	println!("大小写不敏感解析：");
	for input in ["trace", "DEBUG", "Info", "WARN", "warning", "ERROR", "fatal", "critical"] {
...
fn show_trait_from_str() {
	...
	let valid_inputs = ["trace", "debug", "info", "warn", "warning", "error", "fatal", "critical"];
...
fn show_ordering_comparison() {
	...
	let incoming = [LogLevel::Trace, LogLevel::Info, LogLevel::Warn, LogLevel::Error, LogLevel::Fatal];
	for level in incoming {
		let enabled = level >= min_level;
		println!("  {:?} >= {:?} → {} → {}", level, min_level, enabled, if enabled { "记录" } else { "丢弃" });
	}
}
...
fn show_display_trait() {
	...
	for level in [LogLevel::Trace, LogLevel::Debug, LogLevel::Info, LogLevel::Warn, LogLevel::Error, LogLevel::Fatal] {
		...
		println!("  {:?}: Display=\"{}\", as_str=\"{}\", 一致={}", level, display, as_str, consistent);
	}
}
...
fn show_helper_methods() {
	...
	println!("{}", "-".repeat(30));
	for level in [LogLevel::Trace, LogLevel::Debug, LogLevel::Info, LogLevel::Warn, LogLevel::Error, LogLevel::Fatal] {
		println!("{:<10} {:<8} {:<10}", format!("{:?}", level), level.as_str(), level.as_short_str());
	}
...
fn show_error_handling() {
	...
	for input in invalid_inputs {
		...
		match result {
			...
			Err(e) => {
				let msg = format!("{}", e);
				let contains_input = msg.contains(input) || (input.is_empty() && msg.contains("Unknown"));
