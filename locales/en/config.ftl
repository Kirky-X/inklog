...
config-unsafe_path_rejected = Unsafe log path rejected: { $reason }
config-require_dbnexus = require DbNexusModule: { $err }
config-db_not_available = InklogModule on_ready: DbNexusModule not available: { $err }
config-http_startup_failed = HTTP server startup failed (continuing): { $err }
config-shutdown_signal_lost = Shutdown signal lost: worker channel already disconnected
config-http_serialize_failed = Failed to serialize health status: { $err }
...
config-read_header_failed = Failed to read file header
config-read_ciphertext_failed = Failed to read ciphertext
config-write_decrypted_failed = Failed to write decrypted data
config-create_output_failed = Failed to create output file: { $path }
config-get_key_failed = Failed to get encryption key from env var: { $env }
config-encryption_error = Encryption error: { $err }
config-decryption_failed = Decryption failed: { $err }
config-invalid_header = Invalid file header: not an encrypted inklog file
config-unsupported_version = Unsupported file version: { $version }
config-unsupported_algorithm = Unsupported encryption algorithm: { $algo }
config-create_config_failed = Failed to create config file: { $path }
