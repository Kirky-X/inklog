...
本文档介绍 Inklog 企业级 Rust 日志基础设施的整体架构、设计原则、核心组件、数据流与扩展点，帮助使用者和贡献者理解其内部实现。
...
```mermaid
flowchart TD
    App["应用层<br/>(使用 log! 宏的 Rust 应用代码)"]
    API["Inklog API 层<br/>- LoggerManager: 核心协调器<br/>- LoggerBuilder: 流式构建器模式<br/>- 配置管理: 验证、环境变量覆盖<br/>- 健康监控: Metrics, HealthStatus"]
    Sub["LoggerSubscriber 层<br/>- 实现 tracing::Subscriber<br/>- 格式化日志记录<br/>- 数据脱敏处理"]
    Chan1["日志通道队列<br/>Crossbeam bounded channel<br/>(默认容量 10000)"]
    Chan2["控制通道<br/>控制消息、恢复指令<br/>sink 状态查询"]
    Chan3["停止通道<br/>shutdown_tx"]
    Pool["工作线程池<br/>(3 个专用 OS 线程)"]
    W1["文件线程<br/>Sink (阻塞)"]
    W2["DB 线程<br/>Sink (异步)"]
    W3["健康检查线程<br/>每 10 秒检查"]
    W4["轮转线程<br/>(定时器)"]
    FS["FileSink<br/>- 文件、轮转<br/>- 压缩、加密<br/>- 清理"]
    DS["DatabaseSink<br/>- Sea-ORM<br/>- 批量写入、连接池<br/>- 分区、断路器"]
    CS["控制台降级/错误日志<br/>ConsoleSink (File)"]
    Store["存储后端<br/>- 文件系统<br/>- PostgreSQL<br/>- MySQL<br/>- SQLite"]
    HTTP["HTTP 服务器<br/>Axum<br/>/health、/metrics"]

    App --> API --> Sub
    Sub --> Chan1 & Chan2 & Chan3
    Chan1 --> Pool
    Chan2 --> Pool
    Chan3 --> Pool
    Pool --> W1 & W2 & W3 & W4
...
```mermaid
flowchart TD
    subgraph Feature["Feature Layer (inklog)"]
        LM["LoggerManager"]
        LB["LoggerBuilder"]
        LD["LoggerDependencies"]
    end
    subgraph Traits["Infrastructure Traits"]
        CT["Cache trait"]
        CFT["Config trait"]
        DT["Database trait"]
    end
